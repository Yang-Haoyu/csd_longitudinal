...
total_iter = len(args.significant_threshold_lst)*(args.seed_graph_upper - 1)*len(args.N_lst)*len(args.all_bootstrap_seed_lst)*2
con = 0
        ...
        for N in args.N_lst:
            for all_bootstrap_seed in args.all_bootstrap_seed_lst:
                for all_corrupt_level in args.all_corrupt_level_lst:
                    ...
                    print("running {}; The setting is {}".format(model_type, model_name_pc))

                    cwd = os.getcwd()

                    """ =========== load data ============"""
                    data_save_folder = "/{}_{}_{}_{}_{}_{}_{}/".format(
                        seed_graph, N, d, args.followup_min, args.followup_max, args.delta_t, significant_threshold)
                    data_load_path = cwd + "/syn_data" + data_save_folder

                    """ =========== run model ============"""
                    result_dic, log_dic = run_exp(args, data_load_path, model_type, all_corrupt_level=all_corrupt_level,
                                                  all_bootstrap_seed=all_bootstrap_seed)

                    """ =========== save result ============"""
                    save_result(cwd, model_type, log_dic, result_dic, model_name_pc)
