...
total_iter = len( args.significant_threshold_lst) * (args.seed_graph_upper - 1) * len(args.N_lst) * len(args.all_bootstrap_seed_lst) * len(args.all_corrupt_level_lst)
con = 0
        ...
        for N in args.N_lst:
            for all_bootstrap_seed in args.all_bootstrap_seed_lst:
                for all_corrupt_level in args.all_corrupt_level_lst:
                    ...
                    print("running {}; The setting is {}".format(model_type, model_name_fges))
                    ...
                    save_result(cwd, model_type, log_dic, result_dic, model_name_fges)
