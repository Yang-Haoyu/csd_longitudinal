...
import models.notears.utils as ut
...
def main():
    ...
    n, d, s0, graph_type, sem_type = 5000, 5, 7, 'ER', 'mim'
    B_true1 = ut.simulate_dag(d, s0, graph_type)
    np.savetxt('W_true1.csv', B_true1, delimiter=',')

    X = ut.simulate_nonlinear_sem(B_true1, n, sem_type)
    np.savetxt('X1_5000.csv', X, delimiter=',')
    np.savetxt('X1_1000.csv', X[:1000], delimiter=',')
    np.savetxt('X1_200.csv', X[:200], delimiter=',')


    X = np.loadtxt('X1_200.csv', delimiter=',')
    model = NotearsMLP(dims=[d, 10, 1], bias=True)
    W_est1 = notears_nonlinear(model, X, lambda1=0.01, lambda2=0.01)
    assert ut.is_dag(W_est1)
    np.savetxt('W_est1.csv', W_est1, delimiter=',')
    acc = ut.count_accuracy(B_true1, W_est1 != 0)
    print(acc)

    ut.set_random_seed(222)

    n, d, s0, graph_type, sem_type = 5000, 5, 7, 'ER', 'mim'
    B_true2 = ut.simulate_dag(d, s0, graph_type)
    np.savetxt('W_true2.csv', B_true2, delimiter=',')

    X = ut.simulate_nonlinear_sem(B_true2, n, sem_type)
    np.savetxt('X2_5000.csv', X, delimiter=',')
    np.savetxt('X2_1000.csv', X[:1000], delimiter=',')
    np.savetxt('X2_200.csv', X[:200], delimiter=',')

    X = np.loadtxt('X2_1000.csv', delimiter=',')
    model = NotearsMLP(dims=[d, 4, 1], bias=True)
    W_est2 = notears_nonlinear(model, X, lambda1=0.01, lambda2=0.01)
    assert ut.is_dag(W_est2)
    np.savetxt('W_est2.csv', W_est2, delimiter=',')
    acc = ut.count_accuracy(B_true2, W_est2 != 0)
    print(acc)
